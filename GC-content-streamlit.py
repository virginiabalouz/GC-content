...
def calculate_gc_content(sequence, window, step):
    ...
    for i in range(0, sequence_length, step):
        ...
        if len(window_sequence) > 0:
            gc_count = window_sequence.count('G') + window_sequence.count('C') + window_sequence.count('g') + window_sequence.count('c')
...
if fasta_file is not None and not analysis_type:
    ...
    for record in sequences:
        if len(str(record.seq))>=min_len:
            ...
            df_gc_content['Smoothed_GC'] = lowess(df_gc_content["GC_Content"], df_gc_content["Start"], frac=smooth_factor)[:, 1]
...
st.caption("Have feedback, questions or suggestions? I’d love to hear from you — email me at vbalouz@iib.unsam.edu.ar.")
